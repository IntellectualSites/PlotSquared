...
public class MainUtil
{
    ...
    public static boolean isPlotArea(final Location location)
    {
        ...
        if (plotworld.TYPE == 2) { return ClusterManager.getCluster(location) != null; }
        return true;
    }

    public static String getName(final UUID owner)
    {
        if (owner == null) { return C.NONE.s(); }
        final String name = UUIDHandler.getName(owner);
        if (name == null) { return C.UNKNOWN.s(); }
        return name;
    }
    ...
    public static Location getPlotCenter(final Plot plot)
    {
        final Location bot = getPlotBottomLoc(plot.world, plot.id);
        final Location top = getPlotTopLoc(plot.world, plot.id).add(1, 0, 1);
    ...
    public static Plot getPlotFromString(final PlotPlayer player, final String arg, final boolean message)
    {
        ...
        else
        {
            ...
            {
                final String name = p.getSettings().getAlias();
                if ((name.length() != 0) && name.equalsIgnoreCase(arg)) { return p; }
            }
    ...
    public static boolean unlinkPlot(final Plot plot, final boolean createRoad)
    {
    ...
    public static boolean isPlotAreaAbs(final Location location)
    {
        final PlotWorld plotworld = PS.get().getPlotWorld(location.getWorld());
        if (plotworld == null) { return false; }
        if (plotworld.TYPE == 2) { return ClusterManager.getClusterAbs(location) != null; }
        return true;
    }
    ...
    public static boolean isPlotArea(final Plot plot)
    {
        ...
        if (plotworld.TYPE == 2) { return plot.getCluster() != null; }
        return true;
    }
    ...
    public static Location getDefaultHome(final Plot plot)
    {
        final PlotWorld plotworld = PS.get().getPlotWorld(plot.world);
    ...
    {
        final PlotWorld plotworld = PS.get().getPlotWorld(world);
        if (!plotworld.WORLD_BORDER) { return; }
    ...
    public static HashSet<Plot> getPlotSelectionOwned(final String world, final PlotId pos1, final PlotId pos2)
    {
        ...
        if (PS.get().isPlotWorld(world))
        {
            if (size < PS.get().getAllPlotsRaw().get(world).size())
            {
                ...
                {
                    final Plot plot = MainUtil.getPlot(world, pid);
                    if (plot.hasOwner())
                    {
    ...
    {
        if (plotIds.size() < 2) { return false; }
        ...
        final boolean result = EventUtil.manager.callMerge(world, getPlot(world, pos1), plotIds);
        if (!result) { return false; }
    ...
    public static void removeSign(final Plot p)
    {
        ...
        final PlotManager manager = PS.get().getPlotManager(world);
        final PlotWorld plotworld = PS.get().getPlotWorld(world);
        if (!plotworld.ALLOW_SIGNS) { return; }
    ...
    public static void autoMerge(final Plot plot, final UUID uuid, final boolean removeRoads)
    {
        if (plot == null) { return; }
        if (plot.owner == null) { return; }
        if (!plot.owner.equals(uuid)) { return; }
        ArrayList<PlotId> plots;
        boolean merge = true;
        int count = 0;
        final ArrayList<PlotId> toUpdate = new ArrayList<>();
        while (merge)
        {
            if (count > 16)
            {
    ...
    private static boolean ownsPlots(final String world, final ArrayList<PlotId> plots, final UUID uuid, final int dir)
    {
        ...
        for (final PlotId myid : plots)
        {
            ...
            if ((myplot == null) || (myplot.owner == null) || !(myplot.owner.equals(uuid))) { return false; }
            final PlotId top = getTopPlot(myplot).id;
            if (((top.x > id_max.x) && (dir != 1)) || ((top.y > id_max.y) && (dir != 2))) { return false; }
            final PlotId bot = getBottomPlot(myplot).id;
            if (((bot.x < id_min.x) && (dir != 3)) || ((bot.y < id_min.y) && (dir != 0))) { return false; }
        }
        return true;
    }
    ...
    {
        if (!worldBorder.containsKey(plot.world)) { return; }
        final String world = plot.world;
        final PlotManager manager = PS.get().getPlotManager(world);
        final PlotWorld plotworld = PS.get().getPlotWorld(world);
    ...
    public static boolean createPlot(final UUID uuid, final Plot plot)
    {
        ...
        if (PS.get().getPlot(plot.world, plot.id) != null) { return true; }
    ...
    {
        if (runners.containsKey(plot)) { return false; }
    ...
    public static boolean canClaim(final PlotPlayer player, final String world, final PlotId pos1, final PlotId pos2)
    {
        for (int x = pos1.x; x <= pos2.x; x++)
        {
            ...
            {
                final PlotId id = new PlotId(x, y);
                final Plot plot = getPlot(world, id);
                if (!canClaim(player, plot)) { return false; }
            }
        }
        return true;
    }
    ...
    public static boolean canClaim(final PlotPlayer player, final Plot plot)
    {
        ...
        if (Settings.ENABLE_CLUSTERS)
        {
            ...
            if (cluster != null)
            {
                if (!cluster.isAdded(player.getUUID()) && !Permissions.hasPermission(player, "plots.admin.command.claim")) { return false; }
            }
        }
    ...
    public static boolean isUnowned(final String world, final PlotId pos1, final PlotId pos2)
    {
        for (int x = pos1.x; x <= pos2.x; x++)
        {
            for (int y = pos1.y; y <= pos2.y; y++)
            {
                ...
                {
                    if (PS.get().getPlot(world, id).owner != null) { return false; }
                }
            }
        }
        return true;
    ...
    {
        final Plot p1 = PS.get().getPlot(world, current);
        final Plot p2 = PS.get().getPlot(world, newPlot);
        if ((p1 == null) || (p2 == null) || (p1.owner == null) || !p1.owner.equals(p2.owner)) { return false; }
        // Swap blocks
    ...
    public static boolean copy(final String world, final PlotId current, final PlotId newPlot, final Runnable whenDone)
    {
        ...
        for (final PlotId id : selection)
        {
            final int x = id.x + offset_x;
            final int y = id.y + offset_y;
            final Plot plot = createPlotAbs(currentPlot.owner, getPlot(world, new PlotId(x, y)));
    ...
    {
        if (rawString == null) { return new String[] { "" }; }
    ...
    {
        if (!plot.isMerged()) { return new PlotId(1, 1); }
    ...
    public static PlotId getPlotId(final Location loc)
    {
        ...
        {
            if (ClusterManager.getCluster(world, id) == null) { return null; }
        }
        return id;
    }
    ...
    public static double getAverageRating(final Plot plot)
    {
        ...
        if ((rating == null) || (rating.size() == 0)) { return 0; }
        double val = 0;
        int size = 0;
    ...
    public static double[] getAverageRatings(final Plot plot)
    {
        ...
        if ((rating == null) || (rating.size() == 0)) { return ratings; }
        for (final Entry<UUID, Integer> entry : rating.entrySet())
        {
            int current = entry.getValue();
            if ((Settings.RATING_CATEGORIES == null) || (Settings.RATING_CATEGORIES.size() == 0))
    ...
    public static void setComponent(final Plot plot, final String component, final PlotBlock[] blocks)
    {
